...
AM_InsertEntry(fileDesc,attrType,attrLength,value,recId,buff_hits,num_nodes,buff_access)
int fileDesc; /* file Descriptor */
...

{
	...
	status = AM_Search(fileDesc,attrType,attrLength,value,&pageNum,
			   &pageBuf,&index,buff_hits,buff_access);
	...
	if (inserted == FALSE)
	{
		...
		addtoparent = AM_SplitLeaf(fileDesc,pageBuf,&pageNum,
			     attrLength,recId,value, status,index,key,num_nodes);
		...
		if (addtoparent == TRUE)
		{
			errVal = AM_AddtoParent(fileDesc,pageNum,key,attrLength,buff_hits,num_nodes,buff_access);
			if (errVal < 0)
			{
				AM_EmptyStack();
				AM_Errno = errVal;
