...
if (value == NULL)
  {
   ...
   errVal = PF_GetThisPage(fileDesc,AM_LeftPageNum,&pageBuf,bufHits);
   AM_Check;
...
int *bufAccesses;
bufAccesses = (int *) malloc(sizeof (int));
(*bufAccesses)=0;


status = AM_Search(fileDesc,attrType,attrLength,value,&pageNum,&pageBuf,&index,bufAccesses,bufHits);
...
if (index > header->numKeys) 
  ...
  {
  errVal = PF_GetThisPage(fileDesc,header->nextLeafPage,&pageBuf,bufHits);
  AM_Check;
  bcopy(pageBuf,header,AM_sl);
...
errVal = PF_GetThisPage(AM_scanTable[scanDesc].fileDesc
                        ,AM_scanTable[scanDesc].nextpageNum,&pageBuf,bufHits);
AM_Check;
...
while(header->numKeys == 0)
  if(header->nextLeafPage == AM_NULL_PAGE)
   ...
   {
    errVal = PF_GetThisPage(AM_scanTable[scanDesc].fileDesc,header->nextLeafPage,&pageBuf,bufHits);
    AM_Check;
       ...
       if ((AM_scanTable[scanDesc].nextIndex + 1) <= (header->numKeys))
         {
          ...
          if (header->nextLeafPage == AM_NULL_PAGE)
          ...
          else
           {
            ...
            AM_scanTable[scanDesc].actindex = 1;
            errVal =PF_GetThisPage(AM_scanTable[scanDesc].fileDesc,
                  header->nextLeafPage,&pageBuf,bufHits);
            AM_Check;
...
bufHits = (int *) malloc(sizeof (int));
(*bufHits)=0;
errVal = PF_GetFirstPage(fileDesc,&pageNum,&pageBuf,bufHits);
AM_Check;
if (*pageBuf == 'l')
