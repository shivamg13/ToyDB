...
PF_GetFirstPage(fd,pagenum,pagebuf,buff_hits)
int fd;	/* file descriptor */
...
*****************************************************************************/
{
	...
	*pagenum = -1;
	return(PF_GetNextPage(fd,pagenum,pagebuf,buff_hits));
}


PF_GetNextPage(fd,pagenum,pagebuf,buff_hits)
...
PF_GetThisPage(fd,pagenum,pagebuf,buff_hits)
int fd;		/* file descriptor */
...
PFfpage *fpage;
	...
	if ( (error=PFbufGet(fd,pagenum,&fpage,PFreadfcn,PFwritefcn,buff_hits))!= PFE_OK){
